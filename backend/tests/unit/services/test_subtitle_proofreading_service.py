...
class TestSubtitleProofreadingServiceInit:
    ...
    def test_init_with_db_only(self, test_session):
        ...
        When: Creating service without LLM
        Then: Service is created successfully with no LLM
        """
        service = SubtitleProofreadingService(test_session, llm_service=None)
        assert service.db == test_session
        assert service.llm_service is None

    @patch("app.services.subtitle_proofreading_service.MOONSHOT_API_KEY", None)
    def test_init_with_llm_service(self, test_session):
        """
        Given: Database session and mock LLM
        When: Creating service with LLM
        Then: Service uses provided LLM
        """
        mock_llm = Mock()
        service = SubtitleProofreadingService(test_session, llm_service=mock_llm)
...
class TestScanAndCorrect:
    ...
    def test_scan_and_correct_with_mock_llm(self, mock_scan_batch, sample_episode_with_cues):
        ...
        service = SubtitleProofreadingService(test_session, llm_service=Mock())
...
class TestScanBatch:
    ...
    @patch('app.services.subtitle_proofreading_service.OpenAI')
    @patch("app.services.subtitle_proofreading_service.MOONSHOT_API_KEY", "test_key")
    def test_scan_batch_with_llm_success(self, mock_openai, sample_episode_with_cues):
        """
        Given: List of cues and mock LLM response
        When: LLM returns valid JSON
        Then: Returns list of CorrectionSuggestion objects
        """
        episode, cues = sample_episode_with_cues
        test_session = object_session(cues[0])
        ...
        service = SubtitleProofreadingService(test_session, llm_service=None)
        with patch.object(service, 'llm_service', mock_client):
